...
def main():
    ...
    word_embedding_model = models.RoBERTa(args.model_name_or_path, max_seq_length=args.max_seq_length, do_lower_case=True)
...
#"cached_{}_{}_{}_{}".format(  "dev" if evaluate else "train", list(filter(None, args.model_name_or_path.split("/"))).pop(), str(args.max_seq_length),"patent" )


    logger.info("cached file name %s","cached_{}_{}_{}_{}".format(  "dev" if evaluate else "train", list(filter(None, args.model_name_or_path.split("/"))).pop(), str(args.max_seq_length),"patent" ))
