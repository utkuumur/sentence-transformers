...
def train(args, train_dataset, model, train_loss):
    ...
    for epoch in trange(epochs, desc="Epoch", disable=args.local_rank not in [-1, 0]):
        ...
        for step in trange(num_train_objectives * min_batch_size, desc="Iteration", disable=args.local_rank not in [-1, 0]):
            ...
            #print('after mean loss item:', loss_value.item())
            ...
            if args.local_rank in [-1,0] and global_step > 0 and global_step % 5000 == 0:
                model.save(output_path +'_ts_' + str(global_step * 24))
...
def main():
    ...
    word_embedding_model = models.XLNet('xlnet-base-cased', max_seq_length=args.max_seq_length, do_lower_case=True)
